...
```{r full_model}
predict_next_word <- function(string, lambda = 0.4, beta = 0.5) {
    co_scores <- get_co_scores(string)
    eta_co_score <- min(co_scores$co_score, na.rm = TRUE) - 1
    if(eta_co_score==Inf){eta_co_score <- -2}
    ...
    preds <- data.frame(word=pred_rows4$word4, score=pred_rows4$score)
