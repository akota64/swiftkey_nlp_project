...
for those that do not. To reduce the number of parameters we have to train, we will take
$$\eta = min(\beta C_j)-1$$
...
get_co_scores <- function(string){
    ...
    dt <- dt[, avg_log_prob:=rowSums(.SD)/(ncol(dt)-1), .SDcols=2:ncol(dt)][,c("word2","avg_log_prob")]
    dt
...
```{r full_model}
predict_next_word <- function(string, lambda = 0.4) {
